...
then
    ...
    nsenter -n/$NSPATH/$PID/ns/net modprobe ifb
    echo 1
    nsenter -n/$NSPATH/$PID/ns/net ip link add $ext_ingress type ifb
    echo 2
    nsenter -n/$NSPATH/$PID/ns/net ifconfig $ext_ingress up # begin the virtual interface
    echo 3
    # Create ingress on external interface
    nsenter -n/$NSPATH/$PID/ns/net tc qdisc add dev $INTERFACE handle ffff: ingress
    echo 4
    #redirect traffic to ext_ingress
    nsenter -n/$NSPATH/$PID/ns/net tc filter add dev $INTERFACE parent ffff: protocol ip u32 match u32 0 0 action mirred egress redirect dev $ext_ingress
    echo 5
    # nsenter -t $PID -n tc qdisc add dev $ext_ingress root handle 1: htb default 11

fi
echo 6
nsenter -n/$NSPATH/$PID/ns/net tc qdisc del dev $INTERFACE root  #fogify_edge-node-bronx.1.fkmb32v61bzkeutddjhr7ujpl region_bronx  'delay 5ms  rate 100Mbps  loss 0.0001'   'delay 5ms  rate 100Mbps' loss 0.0001'  fkmb32v61b TRUE
echo 7
nsenter -n/$NSPATH/$PID/ns/net tc qdisc del dev $ext_ingress root

#########
# INGRESS
#########
echo 8
nsenter -n/$NSPATH/$PID/ns/net tc qdisc add dev $INTERFACE root netem $RULE
echo 9
#########
# EGRESS
#########
...
nsenter -n/$NSPATH/$PID/ns/net tc qdisc add dev $ext_ingress handle 1: root htb direct_qlen 100000
echo 10

nsenter -n/$NSPATH/$PID/ns/net tc class add dev $ext_ingress parent 1: classid 1:1 htb rate 10000mbit
echo 11
nsenter -n/$NSPATH/$PID/ns/net tc class add dev $ext_ingress parent 1:1 classid 1:11 htb rate 10000mbit
echo 12
nsenter -n/$NSPATH/$PID/ns/net tc qdisc add dev $ext_ingress parent 1:11 handle 10: netem $RULE_OUT
echo 13
